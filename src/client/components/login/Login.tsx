...
const Login: React.FC<LoginProps> = (props) => {
  ...
  let postFunc = async () => {
    ...
    if(props.location.search){
      ...
      try {
         await json(`/auth/verify`, `PUT`, {token: params.id});
      }
      catch (e) {
        throw e;
      }
    ...
    try {
      ...
      console.log(result.role.length)
        if (result.role === "admin") {
          ...
          if(path) {
            if (
                ...
                path.prevPath.includes("/contact")
            ) {
              if (path.prevPath)
                props.history.replace({
                  pathname: path.prevPath,
        ...
        $("#pass").html("You must verify your account!");
        $("#eml").html("You must verify your account!");
        findInvalid(false);
      }
      }
      else {
